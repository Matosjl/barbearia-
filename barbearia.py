...
def registrar_log(acao, detalhes):
    ...
    conn.commit()
    conn.close()
    registrar_log("VENDA", f"{dados['servico']} - {dados['cliente']} pagou R$ {dados['total']:.2f} via {metodo}")
...
def checkout():
    ...
    if metodo == "PIX":
        ...
        st.caption("Escaneie para pagar o barbeiro")
    ...
    if st.button("Confirmar Recebimento"):
        ...
        conn = conectar_banco()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO historico_vendas (horario, cliente, servico, barbeiro, valor_total) VALUES (?,?,?,?,?)",
                       (datetime.now().strftime("%H:%M:%S"), dados['cliente'], dados['servico'], dados['barbeiro'], dados['total']))
        conn.commit()
        conn.close()
        
        registrar_log("VENDA", f"Serviço {dados['servico']} para {dados['cliente']} (Barbeiro: {dados['barbeiro']})")

            # Dentro do if st.button("Confirmar Recebimento"):
...
if login():
    ...
    with aba1:
        st.subheader("Novo Atendimento (Checkout)")
        ...
        with col_b:
            if not df_serv.empty:
                ...
                # Simulação de desconto ou taxa adicional
                ...
                st.warning("Cadastre serviços na aba Configurações primeiro.")
    ...
    with aba2:
        ...
        if not df_hist.empty:
            ...
            # --- TABELA DE FECHAMENTO POR BARBEIRO E FORMA DE PAGAMENTO ---
            st.write("### 🧮 Fechamento por Barbeiro (Por Método)")
            
            # Criando uma tabela dinâmica (Pivot Table)
            # Ela vai mostrar: Barbeiro nas linhas e Forma de Pagamento nas colunas
            # Como não temos a coluna 'metodo' no banco ainda, vamos simular ou garantir que ela exista.
            # NOTA: Se você não adicionou 'metodo' na tabela 'historico_vendas', 
            # você precisará rodar o 'criar_tabelas' atualizado abaixo.
            
            try:
                # Agrupando os dados
                fechamento = df_hist.groupby(['barbeiro', 'metodo_pagamento'])['valor_total'].sum().unstack(fill_value=0)
                
                # Adicionando uma coluna de Total por Barbeiro na direita
                fechamento['TOTAL BARBEIRO'] = fechamento.sum(axis=1)
                
                st.dataframe(fechamento.style.format("R$ {:.2f}"), use_container_width=True)
            except:
                st.warning("⚠️ Para ver o detalhamento por método, certifique-se de que a coluna de pagamento está sendo salva.")
            ...
            with col_graph1:
                st.write("### Faturamento por Barbeiro")
            ...
            st.info("Nenhum atendimento registrado para gerar o financeiro.")

    with aba3:
        st.subheader("Gerenciar Serviços e Sistema")
        
        with st.expander("➕ Adicionar Novo Serviço"):
            n_serv = st.text_input("Nome do Serviço (ex: Corte Degradê)").strip()
            ...
            if st.button("Salvar Serviço"):
                ...
                conn.commit()
                conn.close()
                registrar_log("CONFIG", f"Serviço {n_serv} cadastrado/alterado.")
