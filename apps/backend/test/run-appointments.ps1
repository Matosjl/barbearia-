# Sobe Postgres temp + backend real e roda o smoke de agendamento/atendimento.
$ErrorActionPreference = "Continue"
$ROOT = "C:\BarberProject"; $BE = "$ROOT\apps\backend"
$RESULT = "$BE\test\appointments_result.txt"; "" | Set-Content $RESULT
...
$DATA = Join-Path $env:TEMP "barber_pg_appt"; $PORT = 55445
$srv = $null; $node = $null
...
try {
  ...
  Get-CimInstance Win32_Process -Filter "Name='postgres.exe'" -EA SilentlyContinue | ? {$_.CommandLine -like "*barber_pg_appt*"} | % {Stop-Process -Id $_.ProcessId -Force -EA SilentlyContinue}
  ...
  $env:MIGRATIONS_DIR="$ROOT\database"; $env:JWT_ACCESS_SECRET="t"; $env:JWT_REFRESH_SECRET="t"
  $env:RATE_LIMIT_AUTH="500"
  ...
  $node = Start-Process -FilePath "node" -ArgumentList @("$BE\src\index.js") -WorkingDirectory $BE -WindowStyle Hidden -PassThru -RedirectStandardOutput "$BE\test\server3.log" -RedirectStandardError "$BE\test\server3.err.log"
  ...
  if (-not $ready) { Get-Content "$BE\test\server3.err.log" -EA SilentlyContinue | % { Log $_ }; throw "backend nao subiu" }
  $env:BASE_URL="http://localhost:3000"
  $o = & node "$BE\test\smoke-appointments.mjs" 2>&1; $o | ForEach-Object { Log $_ }
  if ($LASTEXITCODE -ne 0) { Log "`n>>> FALHOU"; Get-Content "$BE\test\server3.err.log" -EA SilentlyContinue | Select-Object -Last 15 | % { Log $_ } }
  else { Log "`n==================== AGENDAMENTO: TUDO PASSOU ====================" }
}
catch { Log "ERRO: $_" }
finally { Cleanup }
